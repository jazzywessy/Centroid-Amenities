...
```{r}
packages = c('rgdal', 'maptools', 'spatstat', 'tmap','raster','tidyverse','sf', 'RColorBrewer', 'classInt','leaflet','ClusterR','rgeos','WeightedCluster','geosphere','dismo','flexclust')
for (p in packages){
...
dplyr:::select(`POSTAL`,`SUBZONE_N`, `PLN_AREA_N`,`1&2Room_Elder_HDB`,`3Room_Elder_HDB`,`4Room_Elder_HDB`,`5Room_Elder_HDB`,`lng`,`lat`,`Total_Elder_HDB`,`Total_Elder_HDB_Scale`)
...
```{r}
subzone_hdb_postal_elder_clean_unfiltered <- subzone_hdb_postal_elder_clean %>% filter(`SUBZONE_N` == "ALJUNIED")
#KAMPONG TIONG BAHRU
subzone <- st_read(dsn = "data", layer = "MP14_SUBZONE_NO_SEA_PL")

subzone_filter <- subzone %>%
filter(SUBZONE_N == "ALJUNIED")

#subzone_hdb_postal_presch_clean_unfiltered.seq <- seqdef(subzone_hdb_postal_presch_clean_unfiltered[, c(8:9)])

# COmpute distance using Hamming distance
#diss <- seqdist(subzone_hdb_postal_presch_clean_unfiltered.seq, method="HAM")

test <- subzone_hdb_postal_elder_clean_unfiltered[,c(8,9)]
testt <- subzone_hdb_postal_elder_clean_unfiltered[,c(10)]


xy <- SpatialPointsDataFrame(test, data.frame(ID=seq(1:length(subzone_hdb_postal_elder_clean_unfiltered$lng))),
      proj4string=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84"))

# use the distm function to generate a geodesic distance matrix in meters
mdist <- distm(xy)

cl <- cclust(test, k=3, save.data=TRUE,weights = subzone_hdb_postal_elder_clean_unfiltered$Total_Elder_HDB,method="hardcl")
